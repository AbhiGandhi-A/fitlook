...
// Helper to remove protocol from domain
const cleanDomain = (domain) => {
    // Strips 'http://' or 'https://' from the start of the string
    return domain ? domain.replace(/^https?:\/\//, '') : '';
};

// --- CRITICAL FIX 1: Use clean domain ---
const SHOPIFY_DOMAIN = cleanDomain(process.env.SHOPIFY_STORE_DOMAIN);


// Initialize Shopify Admin API client
const createShopifyAdminClient = () => {
  return axios.create({
    // Use the cleaned domain here
    baseURL: `https://${SHOPIFY_DOMAIN}/admin/api/2024-01`, 
    headers: {
...
  }
  ...
  // Use the original domain for display since it looks friendlier
...
    // --- CRITICAL FIX 2: Use clean domain ---
